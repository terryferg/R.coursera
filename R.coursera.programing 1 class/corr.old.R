...
rm(list=ls())
setwd("/Users/terryferg/Documents/R.coursera/R.coursera.programing 1 class/specdata")
source("complete.R")
rm
complete.df <- complete("~/Documents/R.coursera/R.coursera.programing 1 class/specdata")
corr <- function(directory, threshold = 0) {
        ...
        setwd(directory) ## sets the directory
        totaldata.df<- data.frame() # sets an empty dataframe as a starting point for rbind
        complete.data.df <- complete("~/Documents/R.coursera/R.coursera.programing 1 class/specdata")  uses 
        list.our.files <<- list.files()  # creates a list of the datafiles in the directory of interest
                for (i in 1:332) {
                totaldata.df <<- rbind(totaldata.df, read.csv(list.our.files[i])) ##loops through the data files rbinding them together
                 }
...
corr("~/Documents/R.coursera/R.coursera.programing 1 class/specdata",150)
object.list <- c("totaldata.df","complete.data.df", "list.our.files", "new.df")


 head(object.list)
tail(complete.data.df)
head(totaldata.df)
